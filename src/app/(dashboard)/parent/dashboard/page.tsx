...
export default async function ParentDashboard() {
  ...
  // Aggregated Stats
  const avgAttendance = student.attendance.reduce((acc: number, curr: any) => acc + curr.percentage, 0) / (student.attendance.length || 1)
  const pendingFees = student.payments.filter((p: any) => p.status === 'PENDING').reduce((acc: number, curr: any) => acc + curr.amount, 0)
    ...
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
           ...
           <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Monitoring Ward: {student.user.name} ({student.regNo})</p>
        </div>
            ...
            <Card className="bg-[#0A0A0B]/80 border-white/10 overflow-hidden backdrop-blur-3xl">
                <CardHeader className="bg-black/40 border-b border-white/5 py-6">
                    <div className="flex items-center justify-between">
                        ...
                        <Badge className="bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 px-3 uppercase font-black text-[9px]">4 New Alerts</Badge>
                    </div>
                </CardHeader>
                            ...
                            <div key={ann.id} className="p-6 hover:bg-white/[0.02] transition-all group cursor-pointer flex gap-6">
                                ...
                                <div className="flex-1 space-y-1">
                                    ...
                                    <div className="flex items-center gap-3 pt-2">
                                        <p className="text-[9px] text-gray-700 uppercase font-black flex items-center gap-1"><Clock className="w-3 h-3" /> 2 hours ago</p>
         ...
         <div className="space-y-8">
            <Card className="bg-gradient-to-br from-[#121214] to-[#0A0A0B] border-white/10 overflow-hidden shadow-2xl">
                 ...
                 <div className="p-6 -mt-16 relative">
                     <div className="w-20 h-20 rounded-3xl bg-indigo-900 border-4 border-black flex items-center justify-center text-white mb-4 shadow-2xl">
                     ...
                     <div className="mt-8 space-y-4">
                        ...
                        <WardInfoRow label="Hostel" value={`${student.hostelBlock} - ${student.hostelRoom}`} />
                        <WardInfoRow label="Batch" value={student.batch} />
                     </div>
...
function StatsCard({ title, value, subtitle, icon, trend, color }: any) {
  const colorMap: any = {
...
function WardInfoRow({ label, value }: any) {
    return (
...
function ComplianceRow({ label, status, color }: any) {
    const colorMap: any = {
