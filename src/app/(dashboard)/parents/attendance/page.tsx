...
import { CheckCircle2, XCircle, Clock, Activity, BookOpen, User, AlertCircle } from "lucide-react"
import { getStudentProfileByParent, getParentWardAttendance, getParentWardAttendanceLogs } from "@/lib/actions"
import { Progress } from "@/components/ui/progress"

export default async function ParentWardAttendancePage() {
  const student = await getStudentProfileByParent()
  const [attendance, logs] = await Promise.all([
    getParentWardAttendance(),
    getParentWardAttendanceLogs()
  ])
  ...
  const overall = attendance.length > 0 
    ? Math.round(attendance.reduce((acc: any, a: any) => acc + a.percentage, 0) / attendance.length) 
    : 0
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
             <Activity className="w-8 h-8 text-emerald-400" /> Ward's Attendance Monitor
          </h1>
          <p className="text-gray-400 mt-1">Real-time attendance tracking for {student.user.name}</p>
        </div>
        <div className="text-right">
             <p className="text-[10px] text-gray-500 uppercase font-bold mb-1">Overall Ward Percentage</p>
      ...
      <Tabs defaultValue="subject-wise" className="w-full">
        <TabsList className="bg-white/5 border border-white/10 p-1 w-full justify-start h-auto flex-wrap mb-6">
          <TabsTrigger value="subject-wise" className="data-[state=active]:bg-emerald-600 data-[state=active]:text-white px-6 py-2 text-xs font-bold uppercase tracking-widest">Subject-wise Breakdown</TabsTrigger>
          <TabsTrigger value="daily-log" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white px-6 py-2 text-xs font-bold uppercase tracking-widest">Daily Session Log</TabsTrigger>
        </TabsList>
